...
#define SPDLOG_TRACE_LOCATION \
	std ::cout << "\033[94m" << __FILE__ << ":" << __LINE__ << " (" << __FUNCTION__ << ")\033[0m" << std::endl;
...
#define SPDLOG_ASSERT_OP(condition, op, ...) \
	do {                                     \
		if (!(condition)) {                  \
...
#define SPDLOG_ASSERT_VOID(condition, ...) \
	do {                                   \
		if (!(condition)) {                \
			SPDLOG_ERROR(__VA_ARGS__);     \
			return;                        \
...
#define SPDLOG_ASSERT_RET(condition, retval, ...) \
	do {                                          \
		if (!(condition)) {                       \
			SPDLOG_ERROR(__VA_ARGS__);            \
			return retval;                        \
